...
def _generate_VCF_header(vcf_file, contig_info: list[tuple]) -> None:
	# General header
	vcf_file.write("##fileformat=VCFv4.2\n")
	vcf_file.write("##source=test_SV_caller")
	vcf_file.write("##fileDate=%s\n"%(time.strftime('%Y-%m-%d %H:%M:%S %w-%Z',time.localtime())))
	for contig in contig_info:
		vcf_file.write("##contig=<ID=%s,length=%d>\n"%(contig[0], contig[1]))
	# ALT
	vcf_file.write("##ALT=<ID=DEL,Description=\"Deletion\">\n")
	vcf_file.write("##ALT=<ID=DUP,Description=\"Duplication\">\n")
	vcf_file.write("##ALT=<ID=DUP:TANDEM,Description=\"Tandem duplication\">\n")
	vcf_file.write("##ALT=<ID=DUP:INV,Description=\"Inverted tandem duplication\">\n")
	vcf_file.write("##ALT=<ID=INV,Description=\"Inversion\">\n")
	vcf_file.write("##ALT=<ID=INS,Description=\"Insertion\">\n")
	vcf_file.write("##ALT=<ID=BND,Description=\"Break end of translocation\">\n")
	# INFO
	vcf_file.write("##INFO=<ID=PRECISE,Number=0,Type=Flag,Description=\"Precise structural variant\">\n")
	vcf_file.write("##INFO=<ID=IMPRECISE,Number=0,Type=Flag,Description=\"Imprecise structural variant\">\n")
	vcf_file.write("##INFO=<ID=SVTYPE,Number=1,Type=String,Description=\"Type of structural variant\">\n")
	vcf_file.write("##INFO=<ID=SVLEN,Number=1,Type=Integer,Description=\"Difference in length between REF and ALT alleles\">\n")
	vcf_file.write("##INFO=<ID=CHR2,Number=1,Type=String,Description=\"Chromosome for END coordinate in case of a translocation\">\n")
	vcf_file.write("##INFO=<ID=END,Number=1,Type=Integer,Description=\"End position of the variant described in this record\">\n")
	vcf_file.write("##INFO=<ID=CIPOS,Number=2,Type=Integer,Description=\"Confidence interval around POS for imprecise variants\">\n")
	vcf_file.write("##INFO=<ID=CILEN,Number=2,Type=Integer,Description=\"Confidence interval around inserted/deleted material between breakends\">\n")
	vcf_file.write("##INFO=<ID=RE,Number=1,Type=Integer,Description=\"Number of read support this record\">\n")
	vcf_file.write("##INFO=<ID=STRAND,Number=A,Type=String,Description=\"Strand orientation of the adjacency in BEDPE format (DEL:+-, DUP:-+, INV:++/--)\">\n")
	vcf_file.write("##INFO=<ID=RNAMES,Number=.,Type=String,Description=\"Supporting read names of SVs (comma separated)\">\n")
	vcf_file.write("##INFO=<ID=AF,Number=A,Type=Float,Description=\"Allele Frequency.\">\n")
	# FILTER
	vcf_file.write("##FILTER=<ID=q5,Description=\"Quality below 5\">\n")
	# FORMAT
	vcf_file.write("##FORMAT=<ID=GT,Number=1,Type=String,Description=\"Genotype\">\n")
	vcf_file.write("##FORMAT=<ID=DR,Number=1,Type=Integer,Description=\"# High-quality reference reads\">\n")
	vcf_file.write("##FORMAT=<ID=DV,Number=1,Type=Integer,Description=\"# High-quality variant reads\">\n")
	vcf_file.write("##FORMAT=<ID=PL,Number=G,Type=Integer,Description=\"# Phred-scaled genotype likelihoods rounded to the closest integer\">\n")
	vcf_file.write("##FORMAT=<ID=GQ,Number=1,Type=Integer,Description=\"# Genotype quality\">\n")


def _write_sv_lines(vcf_file, sv_calls: dict[str, dict[str, list[SVInfo]]]) -> None:
	lines = []
	i = 0
...
def write_vcf_file(samfile: pysam.Samfile, sv_calls: dict, output_path: str) -> None:
	chr_name_list = list()
	contig_info = list()
	...
	with open(output_path, 'w') as vcf_file:
		_generate_VCF_header(vcf_file=vcf_file, contig_info=contig_info)
		vcf_file.write("#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO\tFORMAT\tSAMPLE\n")
		_write_sv_lines(vcf_file=vcf_file, sv_calls=sv_calls)
		vcf_file.close()